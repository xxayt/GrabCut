...
static void help(char** argv)
{
    cout << "\nThis program demonstrates GrabCut segmentation -- select an object in a region\n"
        ...
        "\tb - border matting\n"
        "\n"
        "\tleft mouse button and move - set rectangle\n"
        "\n"
...
int main(int argc, char** argv)
{
    ...
    gcapp.setImageAndWinName(image, winName);  // 设置图像和窗口名称
    gcapp.showImage();  // 显示图像

    for(;;){
        char c = (char)waitKey(0);
        switch (c){
            case '\x1b': // \x1B 表示 ASCII 中的第 1B(27号)字符 ESC
                cout << "Exiting ..." << endl;
            ...
            case 'n':
                ...
                start = clock();
                if(gcapp.nextIter() > iterCount){ //确定好矩形后，可以进行迭代
                    finish = clock();
                    gcapp.showImage();
                    // Mat roiImg = gcapp.saveRect();
                    // if(!roiImg.empty()){
                    //     string savepath = winName +  "_gamma=" + to_string((int)GrabCutSegmentation::gamma) + "_K=" + to_string(GMM::K) + ".jpg";
                    //     cout << "savepath: " << savepath << endl;
                    //     imwrite(savepath, roiImg);
                    // }
